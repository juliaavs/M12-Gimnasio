...
public class ActividadesController implements Initializable {
    ...
    private void manejarToggleActivo() {
        ...
        if (actividadSeleccionada != null) {
            ...
            // Si la actividad se está DESACTIVANDO (nuevoEstado == false)
            if (!nuevoEstado) {
                Alert confirmAviso = new Alert(Alert.AlertType.CONFIRMATION, 
                    "Al desactivar '" + actividadSeleccionada.getNombre() + "', se enviará un aviso de cancelación a todos los alumnos inscritos. ¿Continuar?", 
                    ButtonType.YES, ButtonType.NO);

                Optional<ButtonType> resultAviso = confirmAviso.showAndWait();
                if (resultAviso.get() == ButtonType.NO) {
                    tglActivo.setSelected(true); // Revertir el toggle visualmente
                    return; // Cancelar la operación
                }

                // Enviar notificaciones masivas
                notificacionService.enviarNotificacionCancelacionActividad(
            ...
            if (actividadService.actualizarActividad(actividadSeleccionada)) {
                mostrarAlerta(Alert.AlertType.INFORMATION, "Éxito", 
                    "Estado actualizado. " + (!nuevoEstado ? "Clientes notificados." : ""));
