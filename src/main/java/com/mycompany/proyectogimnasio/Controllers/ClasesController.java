...
public void initialize() {
    ...
    private void handleEliminar() {
        ...
        try {
            int idClase = Integer.parseInt(txtIdClase.getText());
            String nombreActividad = selectedClaseData.get(3);
            String dia = selectedClaseData.get(1);
            String hora = selectedClaseData.get(2);

            // Notificar antes de borrar (o guardar datos en variables)
            notificacionService.enviarNotificacionCancelacionMasiva(idClase, nombreActividad, dia, hora);

            String sql = "DELETE FROM clases WHERE id_clase = ?";
            try (PreparedStatement ps = conn.prepareStatement(sql)) {
                ps.setInt(1, idClase);
                ps.executeUpdate();
                mostrarAlerta(Alert.AlertType.INFORMATION, "Éxito", "Clase eliminada y clientes notificados.");
            }

            // Recargar datos
            clasesData.setAll(cargarTabla()); 
            handleLimpiar();
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void actualizarStatus(String nuevoStatus) {
       ...
       try {
           ...
           // Si el nuevo estado es 'cancelado', notificamos
           if ("cancelado".equalsIgnoreCase(nuevoStatus)) {
               String nombreActividad = selectedClaseData.get(3);
               String dia = selectedClaseData.get(1);
               String hora = selectedClaseData.get(2);
               notificacionService.enviarNotificacionCancelacionMasiva(idClase, nombreActividad, dia, hora);
