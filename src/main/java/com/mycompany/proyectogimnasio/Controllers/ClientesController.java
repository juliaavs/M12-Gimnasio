...
import com.mycompany.proyectogimnasio.Service.NotificacionService;
...
public class ClientesController {
    ...
    private void handleGuardar() {
        ...
        boolean exito = false; // 1. Inicializamos aquí para evitar el error de compilación
        ...
        if (clienteSeleccionado == null) {
            ...
            // 2. Intentamos agregar y obtener el ID
            int idGenerado = clienteService.agregarCliente(nuevoCliente);

            if (idGenerado != -1) {
                exito = true; // Asignamos éxito para que la lógica posterior funcione
                notificacionService.enviarNotificacionBienvenida(idGenerado, nombre);
                mostrarAlerta(true, "Cliente Creado", "Se ha creado el cliente y se le ha enviado una notificación de bienvenida.", "");
            } else {
                mostrarAlerta(false, "Error", "No se pudo guardar el cliente en la base de datos.", "");
            }
        ...
        // 3. Si todo salió bien (ya sea creación o actualización), refrescamos
        if (exito) {
            cargarClientes();
            handleLimpiar();
        }
